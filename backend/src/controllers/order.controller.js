...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  let updatedOrder = await orderCollection.findByIdAndUpdate(
    orderId, // filter
    { status }, // new data
    {
      new: true, // this will return me the updated document
      runValidators: true, // this will validate the new data passing against schema
    }
  );
  if (!updatedOrder) throw new ErrorHandler("order not found", 404);
...
const getAllOrders = asyncHandler(async (req, res) => {
  ...
  if (allOrders.length === 0) throw new ErrorHandler("no orders present", 404);
  res.status(200).json({
    success: true,
